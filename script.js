...
if(urlParams.q != undefined){
	...
	document.getElementById("title").innerHTML = urlParams.q + " | ReadLex Search";
}

//Read ReadLex and save it as an array
var readLex;
function getReadLex(){
	document.getElementById("status").innerHTML = "Downloading ReadLex...";
	var xhttp = new XMLHttpRequest();
	xhttp.onreadystatechange = function(){
		if(this.readyState == 4 && this.status == 200) {
			document.getElementById("status").innerHTML = "Parsing ReadLex...";
	...
	xhttp.open("GET", "Sambahsa-englisch-2021.tsv", true);
	xhttp.send();
}
getReadLex();
...
function searchReadLex(updateURL = false){
	...
	//Update title
	document.getElementById("title").innerHTML = queryInput + " | ReadLex Search";
